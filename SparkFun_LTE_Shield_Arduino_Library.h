...
public:
...
#endif
    boolean begin(HardwareSerial &hardSerial, unsigned long baud = 9600, uint32_t config = SERIAL_8N1, int8_t rxPin = 26, int8_t txPin = 27);
    ...
    LTE_Shield_error_t setModeFormat();
...
private:
    ...
    LTE_Shield_error_t init(unsigned long baud, uint32_t config, int8_t rxPin, int8_t txPin, LTE_Shield_init_type_t initType = LTE_SHIELD_INIT_STANDARD);
